...
async function handlePut(id: string, form: FormData): Promise<Response> {
  ...
  const info = getDb().prepare(
    ...
    title,
    String(form.get('desc_de') ?? ''), String(form.get('desc_en') ?? ''),
    String(form.get('desc_es') ?? ''), String(form.get('desc_it') ?? ''),
    String(form.get('desc_ja') ?? ''), String(form.get('desc_pt') ?? ''),
    JSON.stringify(categories),
    form.get('published') ? 1 : 0, form.get('finished') ? 1 : 0, form.get('online') ? 1 : 0,
    String(form.get('image') ?? ''), String(form.get('url') ?? ''),
    JSON.stringify(languages),
    id
  );

  if (info.changes === 0) return jsonError('Project not found', 404);
