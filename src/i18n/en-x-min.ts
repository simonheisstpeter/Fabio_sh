export default {
  menuHome: "Home",
  menuProjects: "Proj",
  menuAbout: "About",
  menuSocial: "Social",
  menuContact: "Contact",

  name: "Fabio",
  lastname: "Gschweidl",

  contactTitle: "Contact",
  contactName: "Name",
  contactEMail: "Email",
  contactMessage: "Msg",
  contactButton: "Send",
  contactSuccess: "Sent!",
  contactError: "Error. Retry.",
  contactErrName: "Name: 2–100 chars.",
  contactErrEmail: "Invalid email.",
  contactErrMessage: "Msg: 10–2000 chars.",
  contactErrRate: "Too many. Retry later.",
  contactSending: "Sending…",

  website: "Web",
  comingSoon: "Soon...",
  toProjectsLink: "To Proj",
  currentProjects: "Current",
  upcomingProjects: "Upcoming / Paused",
  finishedProjects: "Done",

  custom404Title: "Not found",
  custom404Button: "Back",

  languageSelect: "Lang",

  nowPlaying: "Playing",
  notPlaying: "Idle",

  mediakitTitle: "Media",
  mediakitFonts: "Fonts",
  mediakitColors: "Colors",
  mediakitCursor: "Cursor",
  mediakitLoader: "Load",

  aboutTitle: "About",
  aboutBio: "Fabio Gschweidl. Austria. Many topics.",
  aboutBody: "Frontend. Backend. Sysadmin. Performance. Open protocols. Self-hosting. See projects. Sound system. Tom Selleck. Jean-Paul Belmondo.",
  aboutCv: "Details: /projects. See cats.",
  aboutTidalIntro: "Currently playing:",

  scrollTop: "Top",

  projectsTeaser: "Some current work.",

  searchPlaceholder: "Search...",
  filterAll: "All",
  filterMore: "filts",

  sortLabel: "Sort:",
  sortRecent: "New",
  sortAlpha: "A-Z",

  statusOnline: "Live",
  statusStandby: "Paused",
  statusOffline: "Off",

  cvDownload: "DL",
  cvNoFiles: "No CV yet.",
  cvEnterCode: "Enter code.",
  cvInvalidCode: "Wrong code.",
  cvAccessPlaceholder: "Code",
  cvViewButton: "View",

  socialTitle: "Social",
  socialIntro: "Latest posts. My own server.",
  socialViewProfile: "Full profile",
  socialEmpty: "No posts.",
  socialError: "Feed unavailable.",
  socialAlsoOn: "More",
};
