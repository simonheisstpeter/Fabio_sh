// BCP 47: en-x-cowboy
export default {
  menuHome: "The Homestead",
  menuProjects: "Cattle Drive",
  menuAbout: "The Drifter",
  menuSocial: "Town Square",
  menuContact: "The Telegraph",

  name: "Fabio",
  lastname: "Gschweidl (The Fastest Gun)",

  contactTitle: "Holler at Me",
  contactName: "Your Handle",
  contactEMail: "Mail Coach Address",
  contactMessage: "The Yarn",
  contactButton: "Pull the Trigger ⚡",
  contactSuccess: "Much obliged, partner! I’ll get back to ya.",
  contactError: "Well, tarnation. Something misfired.",

  website: "The Digital Ranch",
  comingSoon: "Still in the saddle...",
  toProjectsLink: "Head to the Stables",
  currentProjects: "Workin’ the Land",
  upcomingProjects: "On the Horizon",
  finishedProjects: "Retired Outlaws",

  custom404Title: "You’re off the map, stranger!",
  custom404Button: "Ride back to Town",

  languageSelect: "Pick your Drawl",

  nowPlaying: "Strummin’ the Banjo",
  notPlaying: "Tumbleweed Silence",

  mediakitTitle: "Branding Iron",
  mediakitFonts: "Wanted Posters",
  mediakitColors: "Sunset Hues",
  mediakitCursor: "The Spur",
  mediakitLoader: "Loading the Wagon",

  aboutTitle: "The Man in Black",
  aboutBio:
    "Fabio Gschweidl, settled in the Austrian plains, ridin' across many different trails in digital media, web infrastructure, and software.",
  aboutBody:
    "From the frontend frontier to backend territories, through sysadmin ranches, open protocol roundups and self-hostin' homesteads, the projects page tells the full tale, partner. Passionate about performance, accessibility, interop and community-driven development. When I ain't workin': sound rig blazin', Tom Selleck on the screen, Jean-Paul Belmondo in the vault.",
  aboutCv: "More details over at the projects page, partner. Them categories'll show ya what I've been wranglin'.",
  aboutTidalIntro:
    "Down below, y'all'll find the audio player showin' what I'm currently listenin' to, whenever somethin's playin'.",

  scrollTop: "Back to the Peak",

  projectsTeaser: "A glimpse at what I'm workin' on.",

  searchPlaceholder: "Search the ranch...",
  filterAll: "All",
  filterMore: "more filters, partner",

  sortLabel: "Sort:",
  sortRecent: "Freshest catch",
  sortAlpha: "A to Z, partner",

  statusOnline: "Ridin'",
  statusStandby: "Holdin' Up",
  statusOffline: "Roped In",

  cvDownload: "Round Up the File",
  cvNoFiles: "No resume's been posted on the board yet, partner.",
  cvEnterCode: "Whisper the passcode to lay eyes on this here resume.",
  cvInvalidCode: "Wrong passcode, stranger. Try again.",
  cvAccessPlaceholder: "The passcode",
  cvViewButton: "Take a Look",

  socialTitle: "The Town Square",
  socialIntro: "My latest word — straight off my own homestead.",
  socialViewProfile: "Mosey on over to the full profile",
  socialEmpty: "Ain't posted nothin' yet.",
  socialError: "Couldn't fetch the feed just now, partner.",
  socialAlsoOn: "Other handles",
};
